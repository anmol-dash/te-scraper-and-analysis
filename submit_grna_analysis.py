#!/usr/bin/env python3
"""
submit_grna_analysis.py
Connect to HPC, upload analysis code (+ optionally the CSV), and submit the gRNA
analysis batch job.
...
If --input is a LOCAL path (file exists on this machine), it is uploaded to the
cluster work directory automatically.  If it is a REMOTE path that does not exist
locally, it is used directly on the cluster without upload.
...
# ── SSH helpers (shared with submit_line_sine_ltr_analysis.py) ────────────────
...
def _connect(hostname, username, password, port=22, key_path=""):
    ...
    if key_path:
        for KeyCls in (paramiko.Ed25519Key, paramiko.RSAKey, paramiko.ECDSAKey):
            try:
                key = KeyCls.from_private_key_file(os.path.expanduser(key_path))
...
def _upload_file(transport, local_path, remote_path):
    sftp = None
    try:
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Submit gRNA analysis batch job to HPC",
    ...
    p.add_argument("--walltime",    default="06:00")
...
def main():
    ...
    # ── Upload input CSV if it's local ────────────────────────────────────────
    ...
    analysis_args = (
        ...
        f"--cas {args.cas} "
        f"--grna-len {args.grna_len}"
    )
    if args.max_candidates:
        analysis_args += f" --max-candidates {args.max_candidates}"
...
echo "[$(date)] GAMECA gRNA analysis started"
echo "  Host: $(hostname)"
...
echo "[$(date)] Running gRNA analysis..."
python -u {shlex.quote(work_dir)}/run_grna_analysis.py {analysis_args}
EXIT_CODE=$?
...
"""
    ...
    _, _, code = _run(
        transport,
        f"cat > {shlex.quote(job_sh)} << 'GAMECA_GRNA_EOF'\n{job_script}\nGAMECA_GRNA_EOF",
    )
    if code != 0:
