#!/usr/bin/env python3
"""
run_presentation_nomotif.py — reagent-design presentation driver WITHOUT motif analysis.

Same as run_iap_presentation.py but skips JASPAR motif enrichment entirely.
...
Runs DIRECTLY ON THE CLUSTER. Given `iapultracombo_tot_counts.csv` it:
  ...
  2. Clusters the per-copy sequences (k-mer -> TruncatedSVD(50) -> embeddings):
       ...
       - 2D PCA (first two SVD components).
       - 3D UMAP rendered twice: with axes and clean (no axes).
  3. Per-cluster MAFFT alignments + consensus sequences (+ CIAlign plots) via
     te_alignment.
  4. gRNA design + coverage/off-target-style analysis via run_grna_analysis.
  5. The reagent-relevant Stage-11 modules (phylo age tree, subfamily structure,
     divergence/landscape, LTR structure, motif-gain, ORF/fold, cluster DE,
     gRNA off-target) via run_stage11_all.
  6. A presentation bundle: per-cluster value table, a cluster x developmental-
     stage expression heatmap, and a markdown explainer.

Independent heavy stages (alignment / gRNA / Stage-11) run in parallel.

Example (cluster):
    python run_presentation_nomotif.py \
        --input iapultracombo_tot_counts.csv \
        --family IAP --assembly mm10 \
        --out-dir iap_pres_nomotif
"""
...
def write_explainer(out_dir, family, cinfo, vals, strand_res):
    ...
    # {family} — reagent-design presentation bundle (no motif)
    ...
    match; gRNA design gives the actual targeting reagents; Stage-11 adds age,
    ORF integrity and off-target context that decides which cluster is worth a
    reagent.
    ...
    ## 4. Stage-11 reagent context  (`stage11/`)
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Reagent-design presentation driver — no motif analysis.",
    ...
    p.add_argument("--out-dir", default="iap_pres_nomotif")
...
def main():
    ...
    print("=" * 64)
    print(f"REAGENT-DESIGN PRESENTATION (no motif)  —  {args.family} / {args.assembly}")
    ...
    # 6. Stage-11 after alignment (consumes consensus FASTA).
