...
under-served is everything that happens once a family has been named: deciding which copies
actually matter, reducing a raw annotation to interpretable per-family evidence, and turning
that answer into reagents a bench scientist can order. We present \textbf{TERRA}, a modular
open-source pipeline that takes a TE family name and a reference assembly and returns two
concrete reagent deliverables, \textbf{family- and locus-specific primers} and
\textbf{allele-aware CRISPRa/i guide RNAs} scored against the full copy landscape with a
coverage-versus-off-target frontier, together with the analytical evidence a bench scientist
needs to choose and interpret them. Reaching those deliverables requires resolving a family
into sub-groups rather than treating it as one unit, because averaging across a family mixes a
minority of expressed, intact copies with a large majority of inert relics. TERRA therefore
clusters copies without alignment, separates the \emph{expressed} consensus from the
\emph{common} consensus, and layers expression, motif, phylogenetic and structural evidence on
each sub-group. Every stage emits figures and machine-readable tables: the cluster embedding,
per-cluster expression profiles with differential-expression tests, MAFFT multiple-sequence
alignments and per-cluster consensus sequences, JASPAR transcription-factor motif enrichment,
and a molecular-clock phylogeny that nominates a master element, alongside the primer and guide
tables themselves. We demonstrate the complete path on
...
The first is determining which copies truly matter. A named family is not a homogeneous
object: it bundles a small number of expressed and intact insertions together with a much
larger number of truncated and inert relics. Analyses that treat the family as one unit
average across all of them, so a real signal carried by a minority of loci is diluted by the
majority that carry none. Resolving the family into sub-groups and then asking which sub-groups
are expressed is therefore not a convenience; it is what makes the downstream result
interpretable, and it produces the per-family evidence a biologist reads directly: the cluster
embedding, per-cluster expression, multiple-sequence alignments and consensus sequences, and
motif enrichment.

The second is that the end point of most TE projects is experimental, which requires
sequence-level reagents, and repetitiveness is exactly what makes them hard to design. A primer
pair must be specific enough to report on the intended copies, and a CRISPRa/i guide will, by
construction, match many copies at once, so its value depends on an explicit trade-off between
how much of the family it covers and how many off-target sites it hits. TERRA treats primers
and guides as first-class outputs rather than an afterthought, and reports the guide trade-off
as a Pareto frontier rather than a single score. Both the reagents and the analytical evidence
behind them, namely the alignments, consensus sequences, expression profiles, motif enrichment
and phylogeny, are returned as figures and tables rather than left for the user to assemble.
...
mean is dominated by the third group, so a real effect carried by the first is 
...
not in general the same as the consensus built from all annotated copies.
...
The partition is a stratification of copy state, not a subfamily assignment. Three
...
\paragraph{Scope.} The core workflow together with the phylogenetics, gRNA, transduction and
...
Around the core walk-through TERRA ships a set of modules that extend the same resolved family
into its regulatory, evolutionary and structural context. Each runs as an independent,
individually-resourced task, consumes the same per-copy tables as the core stages, and emits
its own figures and machine-readable values; every run also writes a provenance manifest (git
SHA, platform, tool versions) alongside the checkpoints that let it resume
(Figure~\ref{fig:stage11overview}). They fall into three groups.

\emph{Regulatory context.} CTCF-site and TAD-boundary proximity flags copies sitting at or near
architectural boundaries, and an epigenetic and regulatory-track overlay intersects copies with
user-supplied signal tracks. Together these separate silenced from active insertions and so
complement the expression stratification of Section~\ref{sec:resolve} with orthogonal
regulatory evidence.

\emph{Evolutionary context.} Orthologous-insertion calling and multi-assembly liftover across
\texttt{hg38}, T2T-CHM13, \texttt{mm10} and \texttt{mm39} place each copy in other assemblies
and species, distinguishing shared ancestral insertions from lineage-specific ones. Automatic
subfamily resolution collapses the per-cluster consensuses into a dendrogram, giving a coarse
view of relatedness above the level of the individual copy.

\emph{Structural context.} A pure-Python LTR structural classifier labels each element
full-length, solo-LTR, internal-only or truncated, and ColabFold structure prediction folds the
consensus ORFs, extending the characterisation from sequence to protein structure and, for the
LTR case, to the intact-element architecture that determines mobilisation competence.

Each of these runs on any family the core workflow accepts and writes the same figure-and-table
output as the core stages, so they slot into the same per-family report.
...
sequence-level predictions that require experimental confirmation.
