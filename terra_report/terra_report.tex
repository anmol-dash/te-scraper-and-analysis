...
\begin{figure}[htbp]
  \centering
  \figorbox[0.92	extwidth]{fig_prototype_journey}
...
\begin{figure}[htbp]
  \centering
  \figorbox[0.78	extwidth]{fig_locus_filtering}
...
\begin{figure}[htbp]
  \centering
  \figorbox[0.95	extwidth]{fig_line_results}
...
\begin{figure}[htbp]
  \centering
  \figorbox[0.62	extwidth]{fig_phylo_master}
...
\begin{figure}[htbp]
  \centering
  \figorbox[0.62	extwidth]{fig_grna_offtarget_pareto}
...
\paragraph{Units.} Expression is consumed as per-locus counts from an external quantifier
...
loci enter the analysis. Differential expression uses pairwise Wilcoxon rank-sum tests with
Benjamini--Hochberg correction across all cluster-pair by stage comparisons ($q<0.05$). Motif
results are counts of JASPAR matches~\citep{castromondragon2022jaspar} per locus intersected with
...
\paragraph{The noise class.} HDBSCAN labels loci it cannot confidently place as $-1$. These are
...
\paragraph{The partition is a stratification of copy state, not a subfamily assignment.} Three
measurements bound what the clusters can be claimed to be, and we report them including the
negative results (Appendix~\ref{sec:appendix}). \emph{Parameter dependence:} across \cvNConfigs{}
configurations of $k$ and \texttt{n\_neighbors} the recovered cluster count ranged from
\cvClustersMin{} to \cvClustersMax{} with a median ARI of \cvMedianARI{} against the adopted
partition, an improvement on the pipeline's superseded default (median ARI \oldMedianARI{},
which admitted only \oldClusters{} clusters and left \oldNoisePct\% unassigned) but short of a
subfamily claim. \emph{Reproducibility:} clustering is deterministic within an environment
(re-running returns identical labels, ARI $1.000$), but UMAP runs with \texttt{n\_jobs}$=-1$, so
cluster counts can shift on hardware with a different core count; every figure here was produced
from the stored labels and embedding rather than re-derived. \emph{Consensus cross-check:} a
consensus was built per cluster and every sampled copy projected onto every consensus with
\texttt{mafft --addfragments --keeplength}; only \xcAccuracy\% of copies are nearest their own
cluster's consensus (chance \xcChance\%), and the median copy is marginally \emph{closer} to
another cluster's consensus (\xcDivOwn{} versus \xcDivOther{}). The clusters therefore do not
correspond to distinct consensus sequences and we do not call them subfamilies anywhere in this
paper. What they do capture, sharply, is degradation state: ORF integrity runs from
\intactRich\% intact in the most intact cluster to \intactPoor\% in the least, and expression
concentrates in the intact-rich clusters. That is the axis the workflow needs, since separating
expressed, intact copies from inert relics is what makes the downstream reagents meaningful.

\paragraph{Scope.} The core workflow plus the phylogenetics, gRNA, transduction and antisense
modules are the tested path and the source of every number reported above. The modules in
Section~\ref{sec:exploratory} run but are not part of the validated result set.
...
The figures below support the results and validation reported in the main text. They are placed here to keep the main narrative on the prototype walk-through and the reagent deliverables; each is referenced from the section it belongs to.
