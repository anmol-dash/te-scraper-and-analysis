...
\graphicspath{{./}{figures/}{reports8/}{reports8/stage11_l1mdt/}{reports8/stage11_b1mus2/}{reports8/stage11_iapltr1mm/}}
...
on MT2\_Mm (mm10) at five \texttt{-{}-max-loci} caps, each a fresh LSF job (job 97784432, 4 cores,
24\,GB requested, single-threaded UCSC/JASPAR fetch as the bottleneck rather than
CPU-parallel stages). Table~\ref{tab:scaling} reports the numbers written by \texttt{query.py} to
\texttt{stage\_times.json} (wall time via \texttt{ru\_maxrss}) together with the two dominant
on-disk artefacts logged at the motif stage: the per-run JASPAR bigBed subset and the
TE$\times$motif overlap table.
...
\begin{table}[h]
  ...
  \begin{tabular}{@{}r r r r r r@{}}
    ...
    \textbf{Loci cap} & \textbf{Sequences} & \textbf{Wall time} & \textbf{Peak RSS} &
      \textbf{JASPAR BED} & \textbf{Overlap table} \\
    \midrule
    ...
    \bottomrule
  \end{tabular}
  \caption{Wall time, peak resident memory, and the two largest per-run on-disk artefacts as a
    function of \texttt{-{}-max-loci}, MT2\_Mm on mm10. The motif/JASPAR stage dominates wall
    time at every size (e.g.\ 8190\,s of the 10522\,s total at $N=2641$), driven by the
    per-run bigBed query against JASPAR (no \texttt{tabix}/\texttt{bgzip} available on this
    node, so the intersect fell back to a full-file \texttt{bedtools} scan); peak memory and
    the overlap-table size both scale roughly linearly with $N$.}
  \label{tab:scaling}
\end{table}

Two follow-on fixes are implied directly by this run. First, \texttt{tabix}/\texttt{bgzip} were
absent on the compute node, so JASPAR intersection fell back to a full-file \texttt{bedtools}
scan at every size instead of the seek-based \texttt{tabix} subsetting the motif stage otherwise
prefers (Section~\ref{sec:pipeline}); provisioning those binaries (or the container image that
bundles them) removes the largest single cost in the table. Second, the JASPAR bigBed subset is
re-downloaded and re-queried per run rather than cached across families on the same assembly,
so repeated sweeps pay the multi-thousand-second query cost every time; the per-run total disk
footprint (beyond the two artefacts above) is now captured directly via \texttt{du} in
\texttt{submit\_scaling\_test.sh} for future sweeps rather than estimated.
...
\begin{figure}[h]
  \centering
  \resizebox{0.85\textwidth}{!}{\figorbox{fig_nextflow_dag}}
...
scheduler (Section~\ref{sec:orchestration}). All numbers below are emitted by the scripts at
