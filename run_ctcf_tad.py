...
def main():
    ...
    # bedtools_closest_distance() drops the -1 "no feature on this contig"
    # sentinel, so a locus missing from tad_dist genuinely has no answer. Keep it
    # NaN rather than folding it into the "not proximal" bucket, and leave
    # boundary_proximal as a nullable boolean so "unknown" survives into the CSV.
    if used_tads:
        df_out["tad_distance_bp"] = [tad_dist.get(n, np.nan) for n in df["name"]]
        df_out["boundary_proximal"] = pd.array(
            [(v <= args.boundary_window) if v == v else None
             for v in df_out["tad_distance_bp"]], dtype="boolean")
        n_resolved = int(df_out["tad_distance_bp"].notna().sum())
        ov.pp(f"  TAD distance resolved for {n_resolved}/{len(df)} copies "
              f"({len(df) - n_resolved} on contigs absent from the boundary file)")
    else:
        df_out["tad_distance_bp"] = np.nan
        df_out["boundary_proximal"] = pd.array([None] * len(df), dtype="boolean")
        n_resolved = 0
    have_tads = bool(n_resolved)
    ...
    # Counted over copies with a resolved distance only — n_resolved is the
    # denominator that belongs next to n_prox, not len(df).
    n_prox = int(df_out["boundary_proximal"].sum()) if n_resolved else 0
    med_dist = float(np.median(list(tad_dist.values()))) if n_resolved else float("nan")
    tex = [
        ...
        rf"\providecommand{{\ctcfUsedTads}}{{{'yes' if have_tads else 'no'}}}",
        rf"\providecommand{{\ctcfNTadResolved}}{{{n_resolved}}}",
        (rf"\providecommand{{\ctcfNBoundaryProx}}{{{n_prox}}}"
         if have_tads else r"\providecommand{\ctcfNBoundaryProx}{---}"),
        (rf"\providecommand{{\ctcfMedTadDistKb}}{{{med_dist/1000:.1f}}}"
         if have_tads else r"\providecommand{\ctcfMedTadDistKb}{---}"),
        "",
    ]
    ...
    txt = [
        ...
        f"  TAD distance known:   "
        f"{f'{n_resolved}/{n}' if used_tads else 'n/a (no TAD file)'}",
        f"  Boundary-proximal:    "
        f"{f'{n_prox}/{n_resolved}' if have_tads else 'n/a (no TAD distances)'}",
        f"  Median TAD distance:  {f'{med_dist/1000:.1f} kb' if have_tads else 'n/a'}",
    ]
