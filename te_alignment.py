...
_CIALIGN_COMPAT_DIR = None


def _cialign_compat_dir():
    """Return a dir holding a sitecustomize.py that restores NumPy 2 removals.

    CIAlign 1.1.4 calls np.in1d (utilityFunctions.removeColumns), removed in
    NumPy 2.0 — under NumPy>=2 every run dies with
    "AttributeError: module 'numpy' has no attribute 'in1d'". We cannot simply
    pin numpy<2.0: colabfold requires >=2.0.2, so the two constraints are
    mutually exclusive.

    CIAlign is a CLI subprocess, so the fix goes in via PYTHONPATH: Python
    imports `sitecustomize` automatically at interpreter startup, before CIAlign
    imports numpy. np.in1d was always documented as equivalent to np.isin for
    1-D input, which is what removeColumns passes, so aliasing is exact here.
    """
    global _CIALIGN_COMPAT_DIR
    if _CIALIGN_COMPAT_DIR and Path(_CIALIGN_COMPAT_DIR, "sitecustomize.py").exists():
        return _CIALIGN_COMPAT_DIR
    import tempfile
    d = Path(tempfile.mkdtemp(prefix="gameca_np_compat_"))
    (d / "sitecustomize.py").write_text(
        "# Auto-generated by GAMECA te_alignment.py — do not edit.\n"
        "# Restores NumPy<2 aliases that CIAlign 1.1.4 still calls.\n"
        "try:\n"
        "    import numpy as _np\n"
    ...
    _CIALIGN_COMPAT_DIR = str(d)
    return _CIALIGN_COMPAT_DIR


def _cialign_env():
    """Environment for CIAlign: Agg backend + the NumPy 2 compatibility shim."""
    import os as _os
    env = {**_os.environ, "MPLBACKEND": "Agg"}
    compat = _cialign_compat_dir()
    existing = env.get("PYTHONPATH", "")
    env["PYTHONPATH"] = f"{compat}:{existing}" if existing else compat
    return env
